<?php
	...
	*/
	...
	//Inserts an activity.
	function addActivity($name, $desc, $time, $day, $colour) {
		$conn = connect();
		  $sql = "insert into activities (name, description, time, day, colour) values ('$name', '$desc', '$time', '$day', '$colour')";
		  $result = $conn->query($sql);
	...
	function updateActivity($id, $name, $description, $time, $day, $colour) {
		$conn = connect();
		$sql = "update activities set name='$name', description='$description', time='$time', day='$day', colour='$colour' where id=$id";
	...
	function db_getActivityColours($userId) {
		$conn = connect();
		$sql = "SELECT schedule.startDate, activities.colour, activities.day FROM schedule INNER JOIN activities ON schedule.activityId = activities.id
where userId=" . $userId . ";";
		$result = $conn->query($sql);
	...
	function db_getUserSchedule($userId) {
		$conn = connect();
		  $sql = "select schedule.id, activities.id AS activitiesId, activities.name, activities.description, activities.day, activities.time, activities.colour, DATE_FORMAT(schedule.startDate, '%e-%c-%Y') as startDate from activities INNER JOIN schedule ON schedule.activityId = activities.id where schedule.userId = " . $userId;
		  $result = $conn->query($sql);
	...
	function db_getUserScheduleAfterDate($userId, $date) {
		$conn = connect();
		$sql = "select schedule.id, activities.id AS activitiesId, activities.name, activities.description, activities.day, activities.colour, DATE_FORMAT(activities.time, '%h:%i') as time, schedule.startDate from activities INNER JOIN schedule ON schedule.activityId = activities.id where schedule.userId = " . $userId . " and schedule.startDate <= '" . $date . "'";
		$result = $conn->query($sql);
